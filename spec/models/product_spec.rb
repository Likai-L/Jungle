...
RSpec.describe Product, type: :model do
  describe 'Validations' do
    it 'error out when creating a new product without a name' do
    ...
    it 'error out when creating a new product without a price' do
      @category = Category.create(name: 'evergreens')
      @product = Product.create(name: 'some_plant', quantity: 1, category: @category)
      pp @product
      expect(@product.errors.full_messages).to include("Price can't be blank")
    end
    it 'error out when creating a new product without a quantity' do
      @category = Category.create(name: 'evergreens')
      @product = Product.create(name: 'some_plant', price: 998, quantity: nil, category: @category)
      expect(@product.errors.full_messages).to include("Quantity can't be blank")
    end
    it 'error out when creating a new product without a category' do
